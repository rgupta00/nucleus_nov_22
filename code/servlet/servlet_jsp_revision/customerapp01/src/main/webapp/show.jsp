...
<table border=1>
    ...
    <c:forEach items="${customerList}" var="customer">
        <tr>
            ...
            <c:if test="${user.profile=='admin'}">
            <td><a href="customers?action=deleteCustomer&id=<c:out value="${customer.id }"/>">delete</a></td>
            </c:if>
            <c:if test="${user.profile=='admin' ||user.profile=='mgr'  }">
            <td><a href="customers?action=updateCustomer&id=<c:out value="${customer.id }"/>">update</a></td>
            </c:if>
        </tr>
    </c:forEach>
    </tbody>
